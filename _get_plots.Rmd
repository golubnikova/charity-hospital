...
data.frame(ICD = data_done$DS %>% str_split(',') %>% unlist) %>%
  ...
  ggplot(aes(x = fct_rev(fct_infreq(ICD))), 
            ...
            ylab('Количество пациентов с данным заболеванием (по визитам)') +
...
data %>%
  ...
  geom_bar(stat = 'identity', fill = wes_palettes$Rushmore1[3]) + 
  theme_bw() +
  labs(x = 'Код МКБ-10', y = 'Количество визитов') + 
  theme(axis.title = element_text(face = 'bold', size = 14),
        legend.title = element_text(face = 'bold', size = 14),
        axis.text = element_text(size = 12),
        axis.text.x = element_text(angle = 45, vjust = 0.6),
...
data %>%
  dplyr::filter(!is.na(redcap_repeat_instrument)) %>%
  dplyr::select(record_id, redcap_repeat_instrument, where.category) %>%
  dplyr::summarise(Count = table(where.category, exclude = NULL),
                   where.category = names(table(where.category, exclude = NULL))) %>%
  dplyr::mutate(where.category = case_when(is.na(where.category) ~ 'нет данных',
                                     T ~ where.category)) %>%
  # Plot 
  ggplot(aes(Count, fct_reorder(where.category, Count) , fill = where.category)) +
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = c("приют" = wes_palettes$FantasticFox1[3], "ночной приют" = wes_palettes$Darjeeling2[2],"стоянка" = wes_palettes$Chevalier1[1], "пункт обогрева" = wes_palettes$Cavalcanti1[5], "медицинский центр" = wes_palettes$Cavalcanti1[3], "удаленная консультация" = wes_palettes$Chevalier1[2], "нет данных" = wes_palettes$Royal1[1])) +
  theme_bw() +
  theme(axis.title = element_text(face = 'bold', size = 14),
        legend.title = element_text(face = 'bold', size = 14),
        axis.text = element_text(size = 12),
        legend.text = element_text(size = 12),
        legend.position = 'none') +
  labs(x  = 'Количество приемов', y = 'Места приема бездомных пациентов')
...
geombar_abs <- data %>%
  dplyr::select(record_id, where.category, starts_with('ds_')) %>%
  dplyr::select(-ends_with('.factor')) %>%
  as.data.table %>%
  filter(!is.na(where.category)) %>%
  melt.data.table(id.vars = c('record_id', 'where.category')) %>%
  dplyr::group_by(where.category) %>%
  dplyr::summarise(icd10 = names(table(value)),
                   Count = table(value)
                   ) %>%
  # Plot
  ggplot(aes(fct_reorder(icd10, Count,.desc = T), Count)) +
  facet_grid(rows = vars(where.category)) +
  geom_bar(stat = 'identity', fill = wes_palettes$Rushmore1[4]) +
  theme_bw() +
  labs(x = 'Заболевания по МКБ-10', y = 'Количество посещений в данной категории') + 
  theme(axis.title = element_text(face = 'bold', size = 12),
...
#data_done$DS <- data_done$DS %>% str_split(',') %>% unlist


geombar_abs.DS <- data_done %>%
  dplyr::select(record_id, where.category.mostfreq, DS) %>%
  dplyr::select(-ends_with('.factor')) %>%
  as.data.table %>%
  filter(!is.na(where.category.mostfreq)) %>%
  melt.data.table(id.vars = c('record_id', 'where.category.mostfreq')) %>%
  dplyr::group_by(where.category.mostfreq) %>%
  dplyr::summarise(icd10 = names(table(value)),
                   Count = table(value)
                   ) %>%
  # Plot
  ggplot(aes(fct_reorder(icd10, Count,.desc = T), Count)) +
  facet_grid(rows = vars(where.category.mostfreq)) +
  geom_bar(stat = 'identity', fill = wes_palettes$Rushmore1[4]) +
  theme_bw() +
  labs(x = 'Заболевания по МКБ-10', y = 'Количество посещений в данной категории') + 
  theme(axis.title = element_text(face = 'bold', size = 12),
...
geombar_pc <- data %>%
  ...
  # Plot
  ggplot(aes(fct_reorder(icd10, perc,.desc = T), perc)) +
  facet_grid(rows = vars(where.category)) +
  geom_bar(stat = 'identity', fill = wes_palettes$Rushmore1[4]) +
  theme_bw() +
  labs(x = 'Заболевания по МКБ-10', y = 'Количество визитов') + 
  theme(axis.title = element_text(face = 'bold', size = 12),
        legend.title = element_text(face = 'bold', size = 12),
...
ds.where <- data.frame(ds = data_done$DS %>% str_split(',') %>% unlist,
           where = rep(data_done$where.category.mostfreq, data_done$DS.N %>% replace_na(1))) %>%
  na.omit
...
chisq.lst <-
  lapply(ds.where$ds %>% table %>% names, function(interval){
  df = ds.where[ds.where$ds == interval,]
  chisq.test(df$where)
}) 

names(chisq.lst) <- ds.where$ds %>% table %>% names
